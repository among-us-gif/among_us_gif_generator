        ...
        for j in range(image.size[1]):
            ...
            if pixel[3] > 0 and not (pixel[0] == pixel[1] and pixel[1] == pixel[2] and pixel[0] == pixel[2]):
                if sum([1 if (value >= 45 and value <= 65) else 0 for value in pixel[:3]]) < 3:
                    new_pixel_green = [int(value * pixel[1] / 255)
                                       for value in color.green]
                    if pixel[1] == 0 or (sum(pixel) >= 508 and sum(pixel) <= 520):
                        new_pixel_red = [int(value * pixel[0] / 255)
                                         for value in color.red]
                        new_pixel_blue = [int(value * pixel[2] / 255)
                    ...
                    new_pixel = [min(
                        new_pixel_red[i] + new_pixel_green[i] + new_pixel_blue[i], 255) for i in range(3)]
...
def apply_layer(base_image, layer_image, layer_origin, base_origin=(0, 0)):
    ...
    new_image = Image.new(base_image.mode, (left+right +
                          base_width, top+bottom+base_height), (0, 0, 0, 0))
...
def generate_all_images(path='scratch', generate_all=True):
    for color in all_colors:
        crop_and_save(generate_base(image='Dead0001.png', color=color),
                      color+'_dead.png', path=path, type='PNG')
        crop_and_save(generate_base(image='Dead0040.png', color=color),
                      color+'_dead_floor.png', path=path, type='PNG')
        crop_and_save(generate_base(image='ghostbob0001.png', color=color),
                      color+'_ghostbob0001.png', path=path, type='PNG')
        crop_and_save(generate_base(image='ejected.png', color=color),
                      color+'_ejected.png', path=path, type='PNG')
        crop_and_save(generate_base(image='idle.png', color=color),
...
def make_square(image):
    ...
    new_image.paste(
        image, (int((size-image.size[0])/2), int((size-image.size[1])/2)), image)
    return new_image
...
def generate_ejection_gif(color='blue', skn='rand', hat=None, text="I have been ejected.", add_stars=True, path='scratch/gifs/', name=None, watermark=True):
    ...
    if not name:
        hasher = hashlib.md5()
        pattern = {'color': color, 'skin': skn, 'hat': hat,
    ...
    for body_x in range(-120, background.size[0]*3, 12):
        ...
        if body_x > int(background.size[0]/2) and body_x <= int(3*background.size[0]/2):
            ...
            image.paste(txt, (int(
                background.size[0]/2-txt.size[0]/2), int(background.size[1]/2-txt.size[1]/2)), txt)
        rotated_body = body.copy().rotate(int(2*body_x/3))
        image.paste(rotated_body, (int(
            body_x-center[0]), int((background.size[1]/2)-center[1])), rotated_body)
        if watermark:
            image.paste(watermark_img, (background.size[0]-watermark_img.size[0]-5,
                        background.size[1]-watermark_img.size[1]-5), watermark_img)
        eject_gif.append(image)

    if not os.path.exists(path):
        os.makedirs(path)
    eject_gif[0].save(os.path.join(path, name+'.gif'), save_all=True,
